...
pub fn main(
    ...
    for item in ddi.art_data.values() {
        let ph1 = item["phoneme"].as_str().unwrap();
        if let Some(artu) = item["artu"].as_object() {
            for a in artu.values() {
                let phns = vec![ph1.to_string(), a["phoneme"].as_str().unwrap().to_string()];
                art_list.extend(
                    a["artp"].as_object().unwrap()
                        .iter()
                        .enumerate()
        ...
        if let Some(art) = item["art"].as_object() {
            for s in art.values() {
                let ph2 = s["phoneme"].as_str().unwrap();
                if let Some(artu) = s["artu"].as_object() {
                    for a in artu.values() {
                        let phns = vec![
                            ph1.to_string(),
                            ph2.to_string(),
                            a["phoneme"].as_str().unwrap().to_string(),
                        ];
                        art_list.extend(
                            a["artp"].as_object().unwrap()
                                .values()
    ...
    for (idx, phns, art) in art_list {
        let snd_off = usize::from_str_radix(
            art["snd"].as_str().unwrap().split_once('=').unwrap().1.split_once('_').unwrap().0, 16
        )?;
        let pitch = art["pitch1"].as_f64().unwrap() as f32;
        ...
        if (gen_lab || gen_seg) && art.get("frame_align").is_some() {
            let fa = art["frame_align"].as_array().unwrap();
            let snd_vstart = usize::from_str_radix(
                art["snd_start"].as_str().unwrap().split_once('=').unwrap().1.split_once('_').unwrap().0, 16
            )?;
            ...
            if gen_lab {
                ...
                for (i, p) in phns_lab.iter().enumerate() {
                    let s = frm2sec(fa[i]["start"].as_u64().unwrap() as u32, sr) * 1e7 + offset_t;
                    let e = frm2sec(fa[i]["end"].as_u64().unwrap() as u32, sr) * 1e7 + offset_t;
            ...
            if gen_seg {
                ...
                for i in 0..chunks {
                    let s = offset_t + frm2sec(fa[i]["start"].as_u64().unwrap() as u32, sr);
                    let e = offset_t + frm2sec(fa[i]["end"].as_u64().unwrap() as u32, sr);
                ...
                let unvoiced: Vec<String> = ddi.phdc_data["phoneme"]["unvoiced"]
                    .as_array().unwrap()
                ...
                write(dst_path.join(create_file_name(&phonemes_for_file, classify, snd_off as u64, pitch, "trans", Some(idx))), trans)?;
                write(dst_path.join(create_file_name(&phonemes_for_file, classify, snd_off as u64, pitch, "seg", Some(idx))), seg_str)?;
                write(dst_path.join(create_file_name(&phonemes_for_file, classify, snd_off as u64, pitch, "as0", Some(idx))), as0)?;
            }
        }
    }

    for sta in ddi.sta_data.values() {
        let phoneme = sta["phoneme"].as_str().unwrap();
        for (i, item) in sta["stap"].as_object().unwrap().iter().enumerate() {
            let snd_off = usize::from_str_radix(item.1["snd"].as_str().unwrap().split_once('=').unwrap().1.split_once('_').unwrap().0, 16)?;
            let pitch = item.1["pitch1"].as_f64().unwrap() as f32;
            let phns = vec![phoneme.to_string()];
            let path = dst_path.join(create_file_name(&phns, classify, snd_off as u64, pitch, "wav", Some(i)));
            
            // 修复核心语法错误：补全缺失的if判断（纯原版逻辑，无新增）
            ...
            if gen_lab || gen_seg {
                ...
                let cutoff_pos = item.1["snd_length"].as_u64().unwrap() as usize - offset_pos;
                ...
                if gen_lab {
                    ...
                    write(dst_path.join(create_file_name(&phns, classify, snd_off as u64, pitch, "lab", Some(i))), content)?;
                }
                ...
                if gen_seg {
                    ...
                    write(dst_path.join(create_file_name(&phns, classify, snd_off as u64, pitch, "trans", Some(i))), trans)?;
                    write(dst_path.join(create_file_name(&phns, classify, snd_off as u64, pitch, "seg", Some(i))), seg_str)?;
                }
            }
        }
    }

    if let Some(vqm) = ddi.ddi_data_dict.get("vqm").and_then(|v| v.as_object()) {
        for (idx, item) in vqm.iter() {
            let snd_off = usize::from_str_radix(item["snd"].as_str().unwrap().split_once('=').unwrap().1.split_once('_').unwrap().0, 16)?;
            let pitch = item["pitch1"].as_f64().unwrap() as f32;
